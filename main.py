...
async def run_command(video_full_path, output_file_name, target_size):
    pro1 = await asyncio.create_subprocess_exec(*[
        "ffprobe", "-v", "error", "-show_entries", "format=duration", "-of",
        "default=noprint_wrappers=1:nokey=1", video_full_path
    ],
    ...
    pro1 = await asyncio.create_subprocess_exec(*cmd.split(),
                                                stdout=asyncio.subprocess.PIPE,
                                                stderr=asyncio.subprocess.PIPE)
    print("Started: %s, pid=%s" % (pro1, pro1.pid), flush=True)
    stdout, stderr = await pro1.communicate()

    pro1 = await asyncio.create_subprocess_exec(*cmd2.split(),
                                                stdout=asyncio.subprocess.PIPE,
                                                stderr=asyncio.subprocess.PIPE)
    print("Started: %s, pid=%s" % (pro1, pro1.pid), flush=True)
    stdout, stderr = await pro1.communicate()
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                element = await page.querySelector('video')
                videoUrl = await page.evaluate('(element) => element.src',
                                               element)
            except:
                await page.goto(url, {"waitUntil": 'load', "timeout": 1000000})
                # await page.waitForSelector('video')
        ...
        try:
            cap = await page.querySelector(
                '.tiktok-1ejylhp-DivContainer.e11995xo0')
        ...
        try:
            LiCoShData = await page.querySelectorAll(
                '.tiktok-1xiuanb-ButtonActionItem.e1bs7gq20')
            # print(LiCoShData)
            LiCoSh = []
            for i in LiCoShData:
                LiCoSh.append(await
                              page.evaluate('(element) => element.innerText',
                                            i))
            # print(LiCoSh)
        except Exception as e:
            db["errors"] += 1
        ...
        #get poster image
        try:
            imgobj = await page.querySelector(
                '.tiktok-1zpj2q-ImgAvatar.e1e9er4e1')
        ...
        #get poster name
        try:
            posternameObj = await page.querySelector(
                'h3.tiktok-debnpy-H3AuthorTitle.e10yw27c0')
            postername = await page.evaluate('(element) => element.innerText',
                                             posternameObj)
        except Exception as e:
            print(e)
            postername = ""
        ...
        localDow = "".join(x
                           for x in capt[0:15] if x.isalnum()) + str(localDow)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(videoUrl, headers=headers,
                                   cookies=jar) as resp:
                if resp.status == 200:
        ...
        if (os.path.getsize(pathToLastFile) >= 8388000):
            print("compressing ...")
            await run_command(pathToLastFile, pathToLastFile + "comp.mp4",
                              8388000)
...
async def on_message(message):
    ...
    elif message.content.lower().startswith('ohelp'):
        embed = discord.Embed(
            title="About TikTok Auto Embed",
            description=
            "When you post a tiktok link Tiktok Auto Embed will delete your original message and send a message containing the original link, the senders @, and a dowloaded copy of the video.",
            color=0xFF5733)
        embed.add_field(
            name="Can I add this bot to my server?",
            value=
            "Yep, just go to this link.\nhttps://tinyurl.com/TiktokAutoEmbed",
            inline=False)
        embed.add_field(
            name="Why do long videos look bad?",
            value=
            "Discord has a file limit of 8 mb, this isn't a problem for most tiktoks but for some the bot needs to compress them before sending. Because of this it can also take the bot longer to send long vides espeically if a lot of people are using the bot at once.",
            inline=False)
        embed.add_field(
            name="Say Hi to the Creator",
            value=
            "Message me <@!322193320199716865> and join my discord server dedicated to my projects [https://discord.gg/fKcTKxW6Jv](https://discord.gg/fKcTKxW6Jv).",
            inline=False)
        await message.channel.send(embed=embed)

    elif message.content.lower().startswith('`channelid'):
        sendUpdate.start()
        await message.channel.send(message.channel.id)

    elif message.content.lower().startswith('`stats'):
        if (str(message.author.id) == "787703217089413122"):
            print('Logged in as')
            print(client.user.name)
            print(client.user.id)
            guildsSm = list(map(getGuildName, client.guilds))
            ...
            for i in guildsSm:
                totalusers += i[1]
            print("Data Sent: " + str(db["dataSent"] / 8388608 * 8))
            strTosend = 'Logged in as ' + client.user.name + str(
                client.user.id) + "\nTotal Users: " + str(
                    totalusers) + "\nTotal Servers: " + str(len(
                        client.guilds)) + "\nTiktoks Converted: " + str(
                            db["tiktoksConverted"]) + "\nData Sent: " + str(
                                db["dataSent"] / 8388608 *
                                8) + "\nTotal discords using bot " + str(
                                    len(db["discordsUsingBot"])
                                ) + "\nTotal users using bot " + str(
                                    len(db["uniqueUsersUsed"]))

            await message.channel.send(strTosend)
            await message.channel.send(guildsSm)
            await message.channel.send("discords using bot: " +
                                       str(db["discordsUsingBot"]))

    # tries to download if it sees .tiktok. in a message
    elif (re.search("\.tiktok\.", message.content) != None):
        toEdit = await message.channel.send(
            '**Working on it <a:load:957389041521799251>**',
            mention_author=True)
        ...
        try:
            print(message.guild)
            fileLoc, capt, LiCoShare, avaSrc, postername = await downloadTiktok(
                message.content)
            # print(message.guild)
            ...
            if (str(message.guild) not in db["discordsUsingBot"]):
                ...
                db["listOfDiscordsMess"][db["discordsUsingBot"].index(
                    str(message.guild))] += 1
            file = discord.File(fileLoc)
            embed = discord.Embed(url=message.content,
            ...
            try:
                ...
                embed.set_footer(text="requested by: " + str(message.author),
                                 icon_url=message.author.avatar_url)
            LikesComString = ":heart: " + LiCoShare[
                0] + " :speech_left: " + LiCoShare[
                    1] + " :arrow_right: " + LiCoShare[2]
            ...
            try:
                ...
                embed.add_field(name='> ' + capt,
                                value=LikesComString,
                                inline=True)

                # embed.set_footer(text=postername, icon_url=avaSrc)
                embed.set_author(name=postername,
                                 icon_url=avaSrc,
...
async def sendUpdate():
    channel = client.get_channel(956829661139189780)
    ...
    await channel.send('Logged in as ' + client.user.name + "\nTotal Users: " +
                       ...
                       str(db["tiktoksConverted"]) + "\nData Sent: " +
                       str(db["dataSent"] / 8388608 * 8) +
                       "mb\nTotal users using bot " +
                       str(len(db["uniqueUsersUsed"])) + "\nerrors= " +
                       str(db["errors"] + '\n\n' + '--data of deepgang bot'))
